...
- Automation must run tests and static analysis inside the `.devcontainer` image (`make test` / `make lint` in the container, or `make docker-run CMD='make test'` from the host) before opening a PR or requesting a human to commit on a protected branch. Do not install Go or `staticcheck` on the host.
