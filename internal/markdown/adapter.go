...
// Tests and callers that do not inject a Fetcher get the sample document.
func FetchDocument(authMode, docID string) (*Document, error) {
	fd, err := SampleFetcher{}.Fetch(context.Background(), authMode, docID)
	if err != nil {
		return nil, err
	}
	return fd.Document, nil
}
